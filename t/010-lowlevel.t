...
{
    my $task1 =  $tnt->call_lua('lp.push', [ 'test', '"data1"' ] => 'lp');
    is $task1->data, 'data1', 'task->data';


    my $task2 =  $tnt->call_lua('lp.push', [ 'test', '"data2"' ] => 'lp');
    ...
    note 'id == 0';
    my $list1 = $tnt->call_lua('lp.subscribe', [ 1, .1, 'abc', 'test' ] => 'lp');
    ...
    note 'id > 0';
    my $list2 = $tnt->call_lua('lp.subscribe',
    ...
    $list3 = $tnt->call_lua('lp.subscribe', [ 0, .1, 'test' ], 'lp');
    is $list3->iter->count, 1, 'items';
    is $list3->iter->get(-1)->id, $task2->id + 1, 'last id';
    is $list3->iter->get(-1)->key, undef, 'no key in last tuple';
...

{
    ...
    my $list = $tnt->call_lua('lp.subscribe', [ 1, .1, 'test' ], 'lp');
...
note 'old interface';
{
    my $tp = $tnt->call_lua('lp.put',
        [ 2, 'abc', 'cde', 'data' ], 'old_lp');


    my $tpc = $tnt->call_lua(
        'lp.take', [ $tp->id, 10, 2 => 'abc', 'cde' ], 'old_lp');
...
}
{
    ...
    my $tpc = $tnt->call_lua(
        'lp.take', [ $tp->id, 10, 2 => 'ab', 'cde' ], 'old_lp');
    is_deeply $tpc->raw, $tp->raw, 'put and take tuples are the same';
